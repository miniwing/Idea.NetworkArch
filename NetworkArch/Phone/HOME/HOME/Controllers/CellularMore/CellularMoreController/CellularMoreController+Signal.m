//
//  CellularMoreController+Signal.m
//  NetworkArch
//
//  Created by Harry on 2021/7/19.
//  Copyright © 2021 Harry. All rights reserved.
//
//  Mail: miniwing.hz@gmail.com
//  TEL : +(852)53054612
//

#import "CellularMoreController+Inner.h"
#import "CellularMoreController+Action.h"
#import "CellularMoreController+Signal.h"
#import "CellularMoreController+Notification.h"
#import "CellularMoreController+Theme.h"
#import "CellularMoreController+Debug.h"

@implementation CellularMoreController (Signal)

#if __Debug__
@def_signal(self);
#endif /* __Debug__ */

@end

#pragma mark - handleSignal
@implementation CellularMoreController (handleSignal)

#if __Debug__
handleSignal(CellularMoreController, selfSignal) {
      
   int                            nErr                                     = EFAULT;
   
   __TRY;
   
   LogDebug((@"-[CellularMoreController selfSignal:] : Signal : %@", aSignal));

   __CATCH(nErr);

   return;
}
#endif /* __Debug__ */

@end
