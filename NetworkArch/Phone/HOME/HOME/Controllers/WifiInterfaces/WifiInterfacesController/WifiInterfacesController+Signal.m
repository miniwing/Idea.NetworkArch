//
//  WifiInterfacesController+Signal.m
//  NetworkArch
//
//  Created by Harry on 2021/8/4.
//  Copyright © 2021 Harry. All rights reserved.
//
//  Mail: miniwing.hz@gmail.com
//  TEL : +(852)53054612
//

#import "WifiInterfacesController+Inner.h"
#import "WifiInterfacesController+Action.h"
#import "WifiInterfacesController+Signal.h"
#import "WifiInterfacesController+Notification.h"
#import "WifiInterfacesController+Theme.h"
#import "WifiInterfacesController+Debug.h"

@implementation WifiInterfacesController (Signal)

#if __Debug__
@def_signal(self);
#endif /* __Debug__ */

@end

#pragma mark - handleSignal
@implementation WifiInterfacesController (handleSignal)

#if __Debug__
handleSignal(WifiInterfacesController, selfSignal) {
      
   int                            nErr                                     = EFAULT;
   
   __TRY;
   
   LogDebug((@"-[WifiInterfacesController selfSignal:] : Signal : %@", aSignal));

   __CATCH(nErr);

   return;
}
#endif /* __Debug__ */

@end
