...
- (void)initDatabase{
    ...
    LogDebug((@"database controller path:%@",databasePath));
...
- (NSArray *)findAllDownload:(NSString *)sql{
   LogDebug((@"database controller findAllDownload start"));
   ...
   LogDebug((@"database controller findAllDownload end"));
...
- (void)pauseAllDownloading{
    ...
    if (SQLITE_OK==sqlite3_exec(self.database, SQL_UPDATE_DOWNLOADING_INFO_STATUS.UTF8String, NULL, NULL, &errorMessage)) {
       LogDebug((@"database controller pauseAllDownloading success"));
    }else{
       LogError((@"database controller pauseAllDownloading fail"));
    }
}
...
- (void)createOrUpdate:(DownloadInfo *)downloadInfo{
    ...
    if (SQLITE_OK==sqlite3_exec(self.database, finalSQL.UTF8String, NULL, NULL, &errorMessage)) {
       LogDebug((@"database controller createOrUpdate success id:%@",downloadInfo.id));
    }else{
       LogError((@"database controller createOrUpdate fail id:%@",downloadInfo.id));
    }
...
- (void)remove:(DownloadInfo *)downloadInfo{
    ...
    char *errorMessage=NULL;
    if (SQLITE_OK==sqlite3_exec(self.database,finalSQL.UTF8String, NULL, NULL, &errorMessage)) {
       LogDebug((@"database controller remove success id:%@",downloadInfo.id));
    }else{
       LogError((@"database controller remove fail id:%@",downloadInfo.id));
    }
}
