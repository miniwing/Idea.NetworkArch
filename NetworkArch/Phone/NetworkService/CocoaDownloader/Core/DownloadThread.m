...
- (void)URLSession:(NSURLSession *)session dataTask:(NSURLSessionDataTask *)dataTask didReceiveResponse:(NSURLResponse *)response completionHandler:(void (^)(NSURLSessionResponseDisposition))completionHandler{
   ...
   LogDebug((@"download thread didReceiveResponse size:%ld uri:%@ path:%@",self.downloadInfo.size,self.downloadInfo.uri,pathUrl.path));
...
- (void)URLSession:(NSURLSession *)session dataTask:(NSURLSessionDataTask *)dataTask didReceiveData:(NSData *)data{
   ...
   LogDebug((@"download thread didReceiveData id:%@ progress:%ld size:%ld self:%@",self.downloadInfo.id,self.downloadInfo.progress,self.downloadInfo.size,self));
...
- (void)URLSession:(NSURLSession *)session task:(NSURLSessionTask *)task didCompleteWithError:(NSError *)error{
   LogDebug((@"download thread didCompleteWithError id:%@ progress:%ld size:%ld error:%@",self.downloadInfo.id, self.downloadInfo.progress,self.downloadInfo.size,error));
    ...
    if (error) {
       ...
       LogDebug((@"download thread didCompleteWithError fail id:%@ progress:%ld size:%ld error:%@",self.downloadInfo.id,self.downloadInfo.progress,self.downloadInfo.size,error));
        
        [self.delegate onDownloadFail];
    } else {
        if (self.downloadInfo.progress==self.downloadInfo.size) {
            
           LogDebug((@"download thread didCompleteWithError success id:%@ progress:%ld size:%ld error:%@",self.downloadInfo.id,self.downloadInfo.progress,self.downloadInfo.size,error));
