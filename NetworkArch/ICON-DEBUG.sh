...
# APPICON_NAME=$TARGET_NAME
APPICON_NAME=AppIcon

echo "APPICON_NAME=$APPICON_NAME"
...
#
# # 是否自动增加build number, "NO" 不自增， "YES" 自增
# # Whether to increase build number automatically, "NO" means false, "YES" means true.
AUTO_INCREASE_BUILD_NUMBER="NO"
#
# # 只在Release环境下，自增build number, 只有在AUTO_INCREASE_BUILD_NUMBER="YES"时有效。"NO" 所有环境都会自增build number，"YES" 只在Release环境下自增build number。
# # In the Release environment only, increase build number automatically, only valid if AUTO_INCREASE_BUILD_NUMBER="YES". "NO" means all environments will increase build number automatically. "YES" means increase build number automatically in Release environment only.
# AUTO_INCREASE_BUILD_NUMBER_ONLY_IN_RELEASE="YES"
#
# # 使用日期当build number，只有在AUTO_INCREASE_BUILD_NUMBER="YES"时有效。
# # Use the date as the build number, only valid if AUTO_INCREASE_BUILD_NUMBER="YES".
# USE_DATE_AS_BUILD_NUMBER="YES"
#
#
#
...
#ICON_BADGE_BACKGROUND_COLOR="rgba(255,0,255,1.0)"
#ICON_BADGE_TEXT_COLOR="rgba(255,255,255,1.0)"
#ICON_BADGE_FONT_SIZE=15
#ICON_BADGE_HEIGHT=20

# ICON_BADGE_BACKGROUND_COLOR="rgba(255,255,255,1.0)"
ICON_BADGE_BACKGROUND_COLOR=$1
# ICON_BADGE_TEXT_COLOR="rgba(255,255,255,1.0)"
# ICON_BADGE_TEXT_COLOR="rgba(255,0,0,1.0)"
ICON_BADGE_TEXT_COLOR=$2
ICON_BADGE_FONT_SIZE=15
ICON_BADGE_HEIGHT=20
...
#ICON_INFO_TEXT_COLOR="rgba(255,0,0,1.0)"
...
#
#
if [ "$NEED_CREATE_NEW_ICONS" = "YES" ];then
...
PBXNATIVE_TARGET=`/usr/libexec/PlistBuddy -c "print :objects:$ROOT_OBJECT:targets:$TARGET_INDEX" $PROJECT_FILE_PATH/project.pbxproj`
CURRENT_TARGETS=`/usr/libexec/PlistBuddy -c "print :objects:$ROOT_OBJECT:targets:" $PROJECT_FILE_PATH/project.pbxproj`
echo "######################################################################"
echo "PBXNATIVE_TARGET is $PBXNATIVE_TARGET"
echo "CURRENT_TARGETS  is $CURRENT_TARGETS"
echo "######################################################################"

for i in ${CURRENT_TARGETS}
do
TEMP_TARGET=`/usr/libexec/PlistBuddy -c "print :objects:$i:name" $PROJECT_FILE_PATH/project.pbxproj`
ITEM=`/usr/libexec/PlistBuddy -c "print :objects:$i" $PROJECT_FILE_PATH/project.pbxproj`
   if [ "$ITEM" != "" ]
   then
      if [ "$TARGET_NAME" = "$TEMP_TARGET" ]
      then
         CURRENT_TARGET=$i
         echo "######################################################################"
         echo "CURRENT_TARGET is $CURRENT_TARGET"
         echo "######################################################################"
      fi
   fi
done
...
######################################################################################################################################################
TEMP_BUILD_CONFIGURATION_LIST=`/usr/libexec/PlistBuddy -c "print :objects:$CURRENT_TARGET:buildConfigurationList" $PROJECT_FILE_PATH/project.pbxproj`
echo "######################################################################"
echo "[HARRY] :: TEMP_BUILD_CONFIGURATION_LIST is $TEMP_BUILD_CONFIGURATION_LIST"
echo "######################################################################"
######################################################################################################################################################
...
######################################################################################################################################################
TEMP_BUILD_CONFIGURATIONS==`/usr/libexec/PlistBuddy -c "print :objects:$TEMP_BUILD_CONFIGURATION_LIST:buildConfigurations" $PROJECT_FILE_PATH/project.pbxproj`
echo "TEMP_BUILD_CONFIGURATIONS is $TEMP_BUILD_CONFIGURATIONS"
######################################################################################################################################################

# Find target configuration.
for i in ${TEMP_BUILD_CONFIGURATIONS}
do
    ITEM=`/usr/libexec/PlistBuddy -c "print :objects:$i" $PROJECT_FILE_PATH/project.pbxproj`
    if [ "$ITEM" != "" ]
    then
...
#
#
if [ "$AUTO_INCREASE_BUILD_NUMBER" = "YES" ]; then
    if [ "$AUTO_INCREASE_BUILD_NUMBER_ONLY_IN_RELEASE" = "YES" ]; then
        if [ $CONFIGURATION != Release ]; then
            exitWithMessage "CONFIGURATION=$CONFIGURATION, don't need increase build number." 0
        fi
    fi

    if [ "$USE_DATE_AS_BUILD_NUMBER" = "YES" ]; then
